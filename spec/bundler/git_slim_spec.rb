...
RSpec.describe Bundler::GitSlim do
  ...
  describe ".prune" do
    ...
    it "does nothing when allowed_files is empty" do
      create_file("lib/main.rb", "# main")

      # With empty allowed_files, build_keep_set only keeps gemspecs
      # But prune will still try to remove everything else
      # This is intentional - caller should check for empty files first
      Bundler::GitSlim.prune(tmpdir, [])

      # Files without gemspec protection get removed
